...
import { Search, Store, Monitor, MessageSquare, Power, Globe, AppWindow, MessageCircle, Trash2, Plus, Shield, Clock, Bell, Wifi, Battery, Volume2 } from 'lucide-react';
...
// Configurazione ottimizzata per il touch
...
function App() {
  ...
  const handleDrop = useCallback((id: string, x: number, y: number, targetId?: string) => {
    if (targetId === 'trash') {
  ...
  const handleAddApp = useCallback((app: AppIcon) => {
    const baseId = app.id;
      ...
      <div className="min-h-screen workspace-gradient overflow-hidden relative">
        ...
        <div className="fixed bottom-0 left-0 right-0 h-12 bg-[#0a1f1d] flex items-center justify-between px-3 shadow-xl z-50">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center bg-[#132927] rounded px-2 py-1">
              <Search className="w-3 h-3 text-white/60 mr-1" />
              <input
                type="text"
                placeholder="Search..."
                className="bg-transparent text-white text-xs outline-none placeholder-white/60 w-32"
              />
            </div>
            <button
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-3 px-3 py-1 bg-[#132927] rounded">
